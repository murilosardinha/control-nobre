class MotorsController < ApplicationController
  ...
  def index
    @q = Product.motor.ransack(params[:q])
    @products = @q.result
      .includes(:filial)
  ...
  def update
    respond_to do |format|
      if @product.update(product_update_params)
        format.html { redirect_to filial_motors_path(@filial), notice: "Motoserra foi atualizado com sucesso." }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to filial_motors_path(@filial), notice: "Motoserra foi deletado com sucesso." }
